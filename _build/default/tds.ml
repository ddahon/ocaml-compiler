...
(* Retourne le résultat de find_all sous la forme d'une option (surcharge) *)
let find_all_opt c nom =
  let res = find_all c nom in
  match res with
  | [] -> None
  | _ -> Some res

let rec chercherGlobalementFonction tds nom =
  match tds with
  | Nulle -> None
  | Courante (m,c) ->
    match find_all_opt c nom with
      | Some _ as li -> li
      | None -> chercherGlobalementFonction m nom
...
let string_of_info info =
  ...
  | InfoFun (n,t,tp,i) -> "Fonction "^n^" : "^(List.fold_right (fun elt tq -> if tq = "" then (string_of_type elt) else (string_of_type elt)^" * "^tq) tp "" )^
                      " -> "^(string_of_type t) ^ "surcharge numero " ^ (string_of_int i)
...
let afficher_globale tds =
 ...
 let modifier_type_fonction_info t tp i =
       match !i with
       |InfoFun(n,_,_,numero_surcharge) -> i:= InfoFun(n,t,tp,numero_surcharge)
